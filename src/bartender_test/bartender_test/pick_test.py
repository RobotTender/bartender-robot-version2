...
class RobotControllerNode(Node):
    ...
    def process_grip(self, items):
        ...
        try:
            ...
            # self.latest_cv_vis = cv2.flip(self.latest_cv_vis, -1)
        ...
        # # deproject (2D + depth -> 3D)
        # point_3d = rs.rs2_deproject_pixel_to_point(self.intrinsics, [u_raw, v_raw], depth_m)
        ...
        print("--- 변환된 최종 3D 좌표 ---")
        # print(f"카메라 좌표: (u={u}, v={v}), Depth: {depth_m*1000:.1f} mm")
        ...
        if self.latest_cv_vis is not None:
            ...
            # cv2.resizeWindow("YOLO Detection Result", 850, 500)

            # cv2.imshow("YOLO Detection Result", vis)
            # cv2.waitKey(0)
    ...
    def move_robot_and_control_gripper(self, x, y, z):
        ...
        from DR_common2 import posx, posj
        try:
            self.get_logger().info("잡기 전 자세 1")
            ...
            self.get_logger().info("잡기 전 자세 2")
